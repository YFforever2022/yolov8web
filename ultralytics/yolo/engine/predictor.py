...
Add some new code on lines 104 and 217 and 118
Usage - sources:
...
class BasePredictor:
    ...
    def write_results_yf(self, results, batch, print_string):
    ...
    def __call__(self, source=None, model=None, stream=False):
        if stream:
            ...
            return self.stream_inference_yf(source, model)
    ...
    # 推理
    def stream_inference_yf(self, source=None, model=None):
        if source is None:
            return ''
        ...
        for batch in self.dataset:
            ...
            with self.dt[2]:
                self.results = self.postprocess(preds, im, im0s)
            abc = self.write_results_yf(0, self.results)  # , (p, im, im0)
            return abc
